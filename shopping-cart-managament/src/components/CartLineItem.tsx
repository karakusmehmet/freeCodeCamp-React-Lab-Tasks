...
const CartLineItem = ({item,dispatch, REDUCER_ACTIONS}: PropsType) : ReactElement => {
    ...
    const onChangeQty = (e:ChangeEvent<HTMLSelectElement>) => {
        dispatch({type: REDUCER_ACTIONS.QUANTİTY, payload:{...item,qty: Number(e.target.value)}})
    }
    ...
    const content = (
        <li className="cart__item">
            ...
            <div className="" aria-label="Price Per Item">{new Intl.NumberFormat('en-US', {style: 'currency', currency: 'USD'}).format(item.price)}</div>
            ...
            <button 
                className="cart__button" onClick={onRemoveFromCart}
