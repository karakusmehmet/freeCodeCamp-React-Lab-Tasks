      ...
      <form method="post" action={"https://superhero-application-form.freecodecamp.org"}>
        ...
        <label className='section column'>
          ...
          <select value={powerSource} onChange={(e) => setPowerSource(e.target.value) }>
            ...
            {powerSourceOptions.map((source) => (<option key={source} value={source}>{source}</option>))}
          </select>
        </label>
        <label className="section column">List your powers (select all that apply):
          {powersOptions.map((power) => (<label key={power}><input type="checkbox" value={power} checked = {powers.includes(power)} onChange={handlePowersChange} /><span>{power}</span></label>))}
        </label>
